...
<%
		...
		//设置分页 
		SplitPage sp = null;
		if(session.getAttribute("asp")==null){
			sp=new SplitPage();
		}else{
			sp=(SplitPage)session.getAttribute("asp");
		}
		sp.setTotalRows(service.getTotalRows());
		String flag=request.getParameter("flag");
		int currentPage=sp.confirmPage(flag);
		session.setAttribute("asp", sp);
		pageContext.setAttribute("articles", service.getAllArticle(sp));
%>
	</head>
	<body>
...
<table>
		...
		<td><a href="/QG/article/allarticle.jsp?flag=<%=SplitPage.FIRSTPAGE%>">首页</a></td>
    	<td><a href="/QG/article/allarticle.jsp?flag=<%=SplitPage.PREVIOUSEPAGE %>">上一页</a></td>     
    	<td><a href="/QG/article/allarticle.jsp?flag=<%=SplitPage.NEXTPAGE %>">下一页</a></td>
    	<td><a href="/QG/article/allarticle.jsp?flag=<%=SplitPage.LASTPAGE %>">最后页</a></td>
    	<td>
    	<form action="/QG/article/allarticle.jsp" method="get">
    	<td>

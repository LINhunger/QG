...
public class SplitPage {
	//单例模式获取SplitPage对象
	static SplitPage sp = new SplitPage();
	public static SplitPage getSplitPage(){
		return sp;
	}
	 //分页请求时,请求标识参数
    ...
    private int pageRows=5;//每页显示记录数,默认5条,
    	 	...
    	 	int newPage=this.currentPage;
           	 if(flag!=null){//flag只可能是下面值之一
           		if(this.totalPages<=0){
           				newPage=1;
           		  }else if(flag.equals(SplitPage.FIRSTPAGE)){
                         newPage=1;
                	  ...
                	  flag=flag.replaceAll("[^0-9]", "");
                	  if(flag.equals("")||Integer.parseInt(flag)<=0){//如果当前页面为负数，跳转到首页
              			newPage=1;
              			this.setCurrentPage(newPage);
              	        return newPage;
              		}
              		if(Integer.parseInt(flag)>=totalPages){//如果当前页面超过总页面数，跳转到尾页
              			newPage=this.totalPages;
              			this.setCurrentPage(newPage);
              	        return newPage;
              		}
